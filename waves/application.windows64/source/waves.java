...
int BACKGROUND_COLOR = color(255);   // background color of the sketch
int PGRAPHICS_COLOR = color(0);
PGraphics pg;

public void setup() {
  
   // higher smooth setting = higher quality rendering
...
public void draw() {
  ...
  float w = PApplet.parseFloat(width) / gridX;
  float h = PApplet.parseFloat(height) / gridY;
  translate(w/2, h/2);
  ...
  for (int y=0; y<gridY; y++) {
    ...
    for (int x=0; x<gridX; x++) {
      ...
      int c = pg.get(PApplet.parseInt(vx), PApplet.parseInt(vy));
...
public void mouseReleased() {
  ...
  public void settings() {  size(1200, 856, P2D);  smooth(8); }
